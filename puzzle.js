...
,
  {name: 'NAND is NOT-AND',
   ...
   outro: '<p>The NAND function is expressed as <span>Z = ~(A & B).</span></p><p>Because a NAND gate combines the functions of an AND gate and a NOT gate, it combines the shape of the AND gate with the bubble from the NOT gate.</p>',
...
,
  {name: 'NOT with NAND',
   ...
   outro: '<p>NAND gates are particularly small and fast in many types of real-world circuits, so it is convenient that they are also so versatile.</p>',
...
,
  {name: 'Multiply by 0-3',
   intro: '<p>Use your new 4-bit fast adder to calculate <b>Z = A &times; N, where N is a 2-bit number.</b></p><p>No path through the circuit may contain more than 6 logic gates. The paths through the 1-bit full adder are treated as 3 gates, and the paths through the 4-bit fast adder are treated as 5 gates.</p>',
   ...
   hint: ['<p>Multiplication by 0 can be done with an AND gate cluster.</p>',
          '<p>A &times; 3 = (A &times; 2) + (A &times; 1).</p>'],
...
,
  {name: 'Multiply by 0-7',
   intro: '<p><b>Z = A &times; N, where N is a 3-bit number.</b></p><p>No path through the circuit may contain more than 9 logic gates. The paths through the 1-bit full adder are treated as 3 gates, and the paths through the 4-bit fast adder are treated as 5 gates.</p>',
   ...
   hint: ['<p>When adding multi-bit numbers, carry propagation becomes a significant component of the critical path. Try to avoid propagating carry more than once.</p>',
          ...
          '<p>If the outputs of multiple full adders are interpreted as a multi-bit bus of S values (S[3:0]) and a multi-bit bus of Cout values (Cout[3:0]), the final sum is S[3:0] + 2&times;Cout[3:0].</p>'],
